#!/usr/bin/env python3
"""
Robust FGSM OCR benchmark on Florence-2 with post-attack defenses.

What this script does:
1. Runs Florence-2 OCR on clean images.
2. Attacks images with FGSM.
...
def run(args) -> None:
    ...
    with open(log_path, log_mode, encoding="utf-8") as log_f:
        ...
        log("Starting robust FGSM OCR recovery run", log_f)
        ...
        attack_tag = f"fgsm_eps{format_eps(args.eps)}"
        summary = {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Robust FGSM OCR defense benchmark for Florence-2-base",
    )
    ...
    parser.add_argument("--output-dir", type=str, default="results_fgsm_florence2_ocr_robust")
