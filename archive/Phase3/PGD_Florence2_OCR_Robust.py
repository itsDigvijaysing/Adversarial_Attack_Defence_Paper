#!/usr/bin/env python3
"""
Robust PGD OCR benchmark on Florence-2 with post-attack defenses.

What this script does:
1. Runs Florence-2 OCR on clean images.
2. Attacks images with iterative PGD.
...
def pgd_attack_ocr(
    ...
    adv_denorm = adv_pixel_values * img_std + img_mean
...
def run(args) -> None:
    ...
    with open(log_path, log_mode, encoding="utf-8") as log_f:
        ...
        log("Starting robust PGD OCR recovery run", log_f)
        ...
        log(
            f"PGD params: eps={args.eps}, alpha={args.alpha}, steps={args.steps}, "
            f"random_start={not args.no_random_start}",
            log_f,
        )
        ...
        attack_tag = f"pgd_eps{format_eps(args.eps)}_s{args.steps}"
        summary = {
            ...
            "config": {
                ...
                "attack_random_start": not args.no_random_start,
                "max_new_tokens": args.max_new_tokens,
                "num_beams": args.num_beams,
                "repetition_penalty": args.repetition_penalty,
                "length_penalty": args.length_penalty,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Robust PGD OCR defense benchmark for Florence-2-base",
    )
    ...
    parser.add_argument(
        "--no-random-start",
        action="store_true",
        help="Disable PGD random start inside the L-inf eps-ball.",
    )
    ...
    parser.add_argument("--output-dir", type=str, default="results_pgd_florence2_ocr_robust")
